...
class Adminmodel extends MY_Model
{
    ...
    {
        $this->DB->group_by('c.id')->join('condition_nutrients n', 'n.condition_id = c.id AND n.nutrient_value > 0 AND n.nutrient_value IS NOT NULL', 'left')
            ->select('c.*, COUNT(DISTINCT n.id) nutrients', FALSE);
